    ...
    <div className="mt-4 pt-4 border-t border-gray-100 space-y-3">
      <div className="flex gap-3">
        <input
          ...
          className="flex-1 px-4 py-2.5 bg-gray-50 border border-transparent rounded-xl text-sm font-semibold focus:outline-none focus:border-blue-300 focus:ring-2 focus:ring-blue-50 transition-all"
        />
        <button
          onClick={handleSave}
      ...
      <textarea
        ...
        rows={2}
        className="w-full px-4 py-2.5 bg-gray-50 border border-transparent rounded-xl text-sm focus:outline-none focus:border-blue-300 focus:ring-2 focus:ring-blue-50 transition-all resize-none"
      />
    </div>
  );
};
...
const AssignmentGrader = () => {
  ...
  if (!assignment) return <div className="p-10 text-gray-500">Assignment not found.</div>;

  const isMCQ = assignment.assignment_type === "mcq";
  const gradedCount = submissions.filter((s) => s.status === "Graded").length;

  return (
    <div className="min-h-screen bg-[#f0f2f5] p-6 lg:p-10 font-antiqua">
      <div className="max-w-5xl mx-auto">
          ...
          <div className="flex-1">
            <div className="flex items-center gap-3 mb-1">
              <span className={`text-[10px] font-black px-3 py-1 rounded-full uppercase tracking-wider ${
                isMCQ ? "bg-blue-50 text-blue-600" :
                assignment.assignment_type === "coding" ? "bg-purple-50 text-purple-600" :
                "bg-orange-50 text-orange-600"
              }`}>
                {assignment.assignment_type}
              </span>
              <span className="text-[10px] font-black px-3 py-1 rounded-full bg-gray-100 text-gray-500 uppercase tracking-wider">
                {assignment.assignment_mode?.replace("_", " ")}
              </span>
            </div>
            <h1 className="text-3xl font-extrabold text-gray-900 leading-tight">{assignment.title}</h1>
        ...
        <div className="grid grid-cols-3 gap-4 mb-8">
          {[
            { label: "Total Submissions", value: submissions.length, icon: HiOutlineClipboardList, color: "text-blue-500 bg-blue-50" },
            { label: "Graded", value: gradedCount, icon: HiOutlineCheckCircle, color: "text-green-500 bg-green-50" },
            { label: "Pending", value: submissions.length - gradedCount, icon: HiOutlineDocumentText, color: "text-yellow-500 bg-yellow-50" },
          ].map(({ label, value, icon: Icon, color }) => (
            <div key={label} className="bg-white rounded-[28px] p-5 shadow-sm flex items-center gap-4">
              ...
              {assignment.questions.map((q, i) => (
                <div key={i} className="flex gap-3 text-sm">
                  ...
                  <div>
                    ...
                    <p className="text-green-600 font-bold mt-0.5">✓ {q.correct_answer}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="space-y-4">
          <h2 className="text-xs font-bold text-gray-400 uppercase tracking-widest border-l-4 border-blue-500 pl-3">
            Student Submissions ({submissions.length})
          </h2>

          {submissions.length === 0 && (
            <div className="bg-white rounded-[32px] p-10 text-center shadow-sm">
              <p className="text-gray-400 font-semibold">No submissions yet.</p>
            </div>
          )}
          ...
          {submissions.map((sub) => {
            ...
            return (
              <div
                key={sub._id}
                className={`bg-white rounded-[28px] p-6 shadow-sm border-l-4 transition-all ${
                  isGraded ? "border-green-400" : "border-gray-100"
                }`}
              >
                <div className="flex items-start justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    {isGraded && (
                      <span className="text-[10px] font-black text-green-600 bg-green-50 px-2 py-1 rounded-lg">GRADED</span>
                    )}
                ...
                {isMCQ && sub.answers?.length > 0 && (
                  <div className="mt-4 pt-4 border-t border-gray-50">
                    <p className="text-xs text-gray-400 font-semibold mb-3 uppercase tracking-widest">Answer Breakdown</p>
                ...
                {sub.faculty_feedback && (
                  <p className="mt-3 text-sm text-gray-500 italic bg-gray-50 rounded-xl px-4 py-2">
                    "{sub.faculty_feedback}"
                  </p>
                )}
              </div>
