...
const StudentDashboard = ({ overrideId }) => {
  ...
  const [assignments, setAssignments] = useState([]); // ← restored from Branch 1 (was missing in Branch 2)

  // ── Weekend UI States (Branch 2) ────────────────────────────────────────────
  ...
  // ── Single source for base URL (Branch 2 approach — cleaner) ────────────────
  ...
  // ── Effect: Fetch Dashboard Data + All Subjects (Branch 2 superset) ──────────
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        // Weekend UI — fetch all subjects for subject picker (Branch 2)
        try {
  ...
  // ── Effect: Fetch Pending Assignments (Branch 1 logic, using API_BASE) ────────
  useEffect(() => {
        ...
        for (const course of timetable) {
          if (!seenSubjectIds.has(course.subject_offering_id)) {
            ...
            const res = await axios.get(
              `${API_BASE}/api/assignment/subject/${course.subject_offering_id}`
            );
            allAssignments.push(...res.data);
          }
        }

        // Deduplicate by assignment _id
        const uniqueAssignments = Array.from(
          new Map(allAssignments.map((a) => [a._id, a])).values()
        );

        // Filter out already-submitted assignments
        const subRes = await axios.get(
          `${API_BASE}/api/submission/student/${auth.studentId}`
        );
        const submittedIds = new Set(
          subRes.data.map((sub) =>
            sub.assignment_id ? sub.assignment_id._id : sub.assignment_id
          )
        );

        const pendingAssignments = uniqueAssignments.filter(
  ...
  return (
    <div className="min-h-screen bg-[#f0f2f5] p-4 md:p-8 font-antiqua relative">
      <div className="max-w-7xl mx-auto">
        ...
        {/* ── WEEKDAY: COURSE CARDS  /  WEEKEND: NO CLASSES UI ── */}
        {timetable.length > 0 ? (
          /* ── WEEKDAY ── */
          <div className="mb-20">
              ...
              {timetable.map((course, index) => (
                ...
                >
                  <div className="flex justify-between items-start mb-8">
                    ...
                    <span
                      className={`text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-widest ${getStatusStyle(
                  ...
                  <div className="mt-8 space-y-4 text-gray-600">
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      <p className="text-sm">
                        Faculty:{" "}
                        <span className="font-bold text-gray-800">{course.faculty_name}</span>
                      </p>
                    </div>
                  </div>
                  ...
                  <button
                    onClick={() =>
                      navigate(`/student/subject-details/${course.subject_offering_id}`)
                    }
                        ...
                        {!notesLoading && subjectNotes.length === 0 && (
                          <div className="bg-white rounded-[32px] p-12 text-center border-2 border-dashed border-blue-200">
                            ...
                            <p className="text-sm text-gray-400 mt-2">
                              Check back later for course notes
                            </p>
                          </div>
                        )}
