...
const HandIn = () => {
    ...
    if (loading) return (
        <div className="min-h-screen flex items-center justify-center bg-[#f0f2f5] dark:bg-slate-900">
            <div className="w-12 h-12 border-4 border-blue-200 dark:border-blue-700 border-t-blue-500 rounded-full animate-spin"></div>
        </div>
    );
    ...
    return (
        <div className="min-h-screen bg-[#f0f2f5] dark:bg-slate-900 p-6 lg:p-12 font-antiqua">
            <div className="max-w-4xl mx-auto">
                {/* Header */}
                <div className="flex items-center space-x-4 mb-10">
                    <Link to={`/student-dashboard/${auth.studentId}`} 
                          className="p-3 bg-white dark:bg-slate-800 rounded-2xl shadow-sm text-gray-600 dark:text-slate-300">
                        <HiOutlineChevronLeft size={24} />
                    </Link>
                    <div>
                        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">{assignment.title}</h1>
                        <p className="text-gray-500 dark:text-slate-400 dark:text-slate-500 font-medium">Due: {new Date(assignment.submission_deadline).toLocaleDateString()}</p>
                    </div>
                </div>
                ...
                {submitted ? (
                    assignment.assignment_type === "mcq" && submissionResult ? (
                        // MCQ Score Card
                        <div className="bg-white dark:bg-slate-800 rounded-[40px] p-12 text-center shadow-lg border-t-8 border-blue-500 animate-in zoom-in duration-300">
                            <div className={`w-24 h-24 rounded-full flex items-center justify-center mx-auto mb-6 text-4xl font-black ${
                                (submissionResult.auto_score || 0) >= 80 ? "bg-green-50 dark:bg-green-900/20 text-green-500" :
                                (submissionResult.auto_score || 0) >= 50 ? "bg-yellow-50 text-yellow-500" :
                                "bg-red-50 dark:bg-red-900/30 text-red-500"
                            }`}>
                                {Math.round(submissionResult.auto_score || 0)}%
                            </div>
                            <h2 className="text-3xl font-extrabold text-gray-800 dark:text-white">Assignment Complete!</h2>
                            <p className="text-gray-500 dark:text-slate-400 dark:text-slate-500 mt-2 font-medium">
                                You got <span className="font-black text-gray-800 dark:text-white">
                                    {submissionResult.answers?.filter(a => a.is_correct).length || 0}
                                </span> out of <span className="font-black text-gray-800 dark:text-white">{assignment.questions.length}</span> correct.
                            </p>
                            <div className="mt-8 grid grid-cols-2 gap-3 max-w-xs mx-auto">
                                {submissionResult.answers?.map((ans, i) => (
                                    <div key={i} className={`px-4 py-2 rounded-2xl text-sm font-bold flex items-center gap-2 ${
                                        ans.is_correct ? "bg-green-50 dark:bg-green-900/20 text-green-600 dark:text-green-400" : "bg-red-50 dark:bg-red-900/30 text-red-500"
                                    }`}>
                        ...
                        <div className="bg-white dark:bg-slate-800 rounded-[40px] p-12 shadow-lg border-t-8 border-green-500 animate-in zoom-in duration-300">
                            ...
                            <div className="mb-8 max-w-xl mx-auto">
                                <label className="block text-sm font-bold text-gray-700 dark:text-slate-200 uppercase tracking-widest mb-3 text-left">Submitted Drive Link / Repository</label>
                                <div className="flex space-x-4">
                                    <input 
                                        ...
                                        className="w-full bg-gray-50 dark:bg-slate-700 text-gray-800 dark:text-white px-6 py-4 rounded-2xl outline-none focus:ring-4 focus:ring-blue-100 font-medium transition-all"
                                    />
                                    {!isPastDeadline && (
                                        <button 
                    ...
                    <div className="space-y-8">
                        {/* Instructions Card */}
                        <div className="bg-white dark:bg-slate-800 rounded-[40px] p-8 shadow-sm border-l-8 border-blue-500">
                            <h3 className="text-lg font-bold text-gray-800 dark:text-white mb-4 flex items-center">
                                <HiOutlineDocumentText className="mr-2 text-blue-500 dark:text-blue-400" /> Instructions
                            </h3>
                        ...
                        {assignment.assignment_type === "mcq" && (
                            <div className="space-y-6">
                                {assignment.questions.map((q, qIdx) => (
                                    <div key={qIdx} className="bg-white dark:bg-slate-800 rounded-[32px] p-8 shadow-sm group transition-all">
                                        <div className="flex items-start space-x-4 mb-6">
                                            <span className="w-8 h-8 bg-blue-50 dark:bg-blue-900/40 text-blue-600 dark:text-blue-400 rounded-lg flex items-center justify-center font-black text-sm shrink-0 mt-0.5">
                                                {qIdx + 1}
                                            </span>
                                            <p className="text-lg font-bold text-gray-800 dark:text-white leading-snug">{q.question_text}</p>
                                        </div>
                        ...
                        {assignment.assignment_type !== "mcq" && assignment.questions && assignment.questions.length > 0 && (
                            <div className="space-y-6">
                                {assignment.questions.map((q, qIdx) => (
                                    <div key={qIdx} className="bg-white dark:bg-slate-800 rounded-[32px] p-8 shadow-sm group transition-all border border-gray-100 dark:border-slate-600 border-l-8 border-l-blue-500">
                        ...
                        {assignment.assignment_type !== "mcq" && (
                            <div className="bg-white dark:bg-slate-800 rounded-[40px] p-10 shadow-sm text-center border-2 border-dashed border-gray-200 dark:border-slate-600">
                                <div className="w-16 h-16 bg-blue-50 dark:bg-blue-900/40 text-blue-500 dark:text-blue-400 rounded-2xl flex items-center justify-center mx-auto mb-6">
                                    <HiOutlineCloudUpload size={32} />
                                </div>
                                <h3 className="text-xl font-bold text-gray-800 dark:text-white mb-2">Upload Submission</h3>
                                <p className="text-gray-500 dark:text-slate-400 dark:text-slate-500 mb-8 font-medium italic">Paste your Google Drive or Repository link below</p>
                                <input 
                                    type="url"
                                    placeholder="https://drive.google.com/..."
                                    value={driveLink}
                                    onChange={(e) => setDriveLink(e.target.value)}
                                    className="w-full max-w-lg mx-auto px-6 py-4 bg-gray-50 dark:bg-slate-700 border border-transparent rounded-2xl outline-none focus:bg-white dark:bg-slate-800 focus:border-blue-200 dark:border-blue-700 focus:ring-4 focus:ring-blue-50 transition-all text-center font-medium"
                                />
                            </div>
                        )}
                        ...
                        {!isPastDeadline ? (
                            <button
                                ...
                                className={`w-full py-6 rounded-[32px] text-xl font-black shadow-2xl transition-all active:scale-95 ${submitting ? "bg-gray-200 text-gray-400 dark:text-slate-500 dark:text-slate-400 dark:text-slate-500" : "bg-blue-600 text-white hover:bg-blue-700"}`}>
                                {submitting ? "Processing Hand-in..." : "Submit Assignment"}
                            </button>
                        ) : (
                            <div className="w-full py-6 rounded-[32px] text-xl font-black shadow-lg bg-red-50 dark:bg-red-900/30 text-red-500 flex items-center justify-center">
