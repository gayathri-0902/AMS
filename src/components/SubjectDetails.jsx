...
const SubjectDetails = () => {
  ...
  const getDaysLeft = (deadline) => {
    ...
    return { label: `${days}d left`, color: days <= 3 ? "bg-orange-100 text-orange-600" : "bg-blue-50 text-blue-600" };
  };
            ...
            <div className="bg-white rounded-[40px] p-8 shadow-sm border border-gray-100 text-left">
              <h3 className="text-xl font-bold text-gray-800 mb-6 flex items-center">
                ...
                {assignments.length > 0 && (
                  <span className="ml-2 bg-blue-50 text-blue-600 text-xs font-black px-2 py-0.5 rounded-full">{assignments.length}</span>
                )}
              </h3>
              <div className="space-y-4">
                {assignments.length > 0 ? assignments.map((task) => {
                  ...
                  return (
                    <div key={task._id} className="p-5 bg-gray-50 rounded-[24px] border border-transparent hover:border-blue-100 transition-all">
                      <div className="flex justify-between items-start mb-2">
                        <div className="flex items-center gap-2 flex-1 mr-2">
                          <span className={`text-[10px] font-black px-2 py-0.5 rounded-md uppercase tracking-wider shrink-0 ${
                            task.assignment_type === 'mcq' ? 'bg-blue-100 text-blue-600' :
                            task.assignment_type === 'coding' ? 'bg-purple-100 text-purple-600' :
                      ...
                      <p className="text-sm text-gray-500 mb-4 line-clamp-2">{task.instructions}</p>
