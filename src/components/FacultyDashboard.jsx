          ...
          <div className="flex items-center space-x-4 p-2">
            ...
            <div className="text-left">
              <h2 className="text-2xl font-bold text-gray-800 leading-none tracking-tight">
                {capitalizeName(auth?.name || auth?.facultyName) ||
                  "Faculty Member"}
              </h2>
        ...
        <div className="mb-10 text-left px-2">
          ...
          <p className="text-xl text-gray-500 mt-1">
            ...
            <span className="text-[#3b82f6] font-bold">
              {capitalizeName(auth?.name?.split(" ")[0]) || "Professor"}
            </span>
          </p>
        </div>
                    ...
                    {allSubjects.map((sub) => (
                      ...
                      >
                        <div className="flex justify-between items-start mb-4">
                          <div
                            className={`w-12 h-12 rounded-xl flex items-center justify-center ${selectedSubjectOffering?.subject_offering_id === sub.subject_offering_id ? "bg-blue-500 text-white" : "bg-blue-50 text-blue-500"}`}
                          >
              ...
              {classes.map((cls) => (
                ...
                >
                  <div className="flex justify-between items-start mb-6">
                    <div
                      className={`w-14 h-14 rounded-2xl flex items-center justify-center transition-colors ${selectedClass?.class_id === cls.class_id ? "bg-blue-500 text-white" : "bg-blue-50 text-blue-500"}`}
                    >
                  ...
                  <div className="mt-8 space-y-3">
                    ...
                    <div className="flex items-center space-x-3 text-gray-600">
                      ...
                      <p className="text-sm font-bold text-gray-700">
                        ...
                        {formatTime(cls.end_time || "Next Slot")}
                      </p>
                    </div>
                  </div>
                </div>
            ...
            {selectedClass && (
              <div className="bg-white rounded-[40px] p-10 shadow-sm border border-gray-100">
                <div className="flex justify-between items-center mb-8">
                  ...
                  <button
                    onClick={() => setFaceDetectionMode(!faceDetectionMode)}
                  ...
                  >
                    {faceDetectionMode ? "Face Detection" : "Manual"}
                  </button>
                </div>
                ...
                {faceDetectionMode ? (
                  ...
                  <div className="space-y-6">
                    <div className="bg-blue-50 p-4 rounded-xl">
                      <h4 className="font-bold text-blue-800 mb-2">Face Detection Workflow:</h4>
                      <ol className="text-sm text-blue-700 space-y-1">
                        <li>1. Click "Reset Session" to start</li>
                        <li>2. Upload multiple class photos</li>
                        <li>3. Check status to see recognition progress</li>
                        <li>4. Finalize to mark attendance automatically</li>
                      </ol>
                    </div>
                    ...
                    {faceStatus && (
                      <div className="bg-gray-50 p-4 rounded-xl">
                        ...
                        <p>Photos Processed: {faceStatus.total_frames}</p>
                        <p>Students Tracked: {faceStatus.students_tracked}</p>
                        <div className="mt-2">
                          <h5 className="font-semibold">Recognition Counts:</h5>
                          <pre className="text-xs bg-white p-2 rounded">{JSON.stringify(faceStatus.current_counts, null, 2)}</pre>
                        </div>
                      </div>
                    )}
                    ...
                    <div className="space-y-3">
                      <input
                        type="file"
                        accept="image/*"
                        onChange={(e) => setFacePhotoFile(e.target.files[0])}
                        className="w-full p-3 border border-gray-200 rounded-xl"
                      />
                      <button
                        onClick={handleFacePhotoUpload}
                        disabled={!facePhotoFile}
                        className="w-full bg-green-500 text-white py-3 rounded-xl font-bold hover:bg-green-600 disabled:bg-gray-300 transition-all"
                      >
                        Upload Photo
                      </button>
                    </div>
                    ...
                    >
                      {attendanceMarked ? "✓ Attendance Finalized" : "Finalize Attendance"}
                    </button>
                  </div>
                ) : (
                  // Manual Attendance UI
                  <>
                    <button
                  ...
                  <div className="space-y-3 max-h-[450px] overflow-y-auto pr-2 mb-8 border-t border-gray-100 pt-6">
                    {students.map((student) => (
                      <div
                        key={student._id}
                        className="flex justify-between items-center p-5 bg-gray-50 rounded-2xl border border-transparent hover:border-blue-100 transition-all"
                      >
            ...
            >
              ...
              <div className="bg-white rounded-[40px] p-10 shadow-sm border-t-8 border-purple-500 border-x border-b border-gray-100">
                <h3 className="text-2xl font-bold mb-6 flex items-center text-gray-800">
                  <HiOutlineCloudUpload className="mr-3 text-purple-600 w-8 h-8" />{" "}
                  Upload Resources
                </h3>
                {selectedSubjectOffering && (
                  <p className="text-sm font-bold text-gray-400 mb-6 uppercase tracking-widest bg-purple-50 p-3 rounded-xl inline-block">
                    ...
                    <span className="text-purple-600">
                      {selectedSubjectOffering.course_name}
                    </span>
                  </p>
                )}
                <div
                  onClick={() => noteFileRef.current?.click()}
                  className="border-2 border-dashed border-purple-100 rounded-[24px] p-10 flex flex-col items-center justify-center bg-purple-50/30 cursor-pointer mb-6 hover:bg-purple-50 transition-colors group"
                >
                  <input
                    type="file"
                    ref={noteFileRef}
                    className="hidden"
                    onChange={(e) => {
                      setSelectedNoteFile(e.target.files[0]);
                      setNewNote({
                        ...newNote,
                  ...
                  <p className="text-sm font-bold text-purple-400 uppercase tracking-widest text-center">
                    {selectedNoteFile
                      ? selectedNoteFile.name
                      : "Choose PDF/DOC/TXT to Upload"}
                  </p>
                </div>
                <form onSubmit={handleAddNote}>
                  <input
                    ...
                    className="w-full px-6 py-4 bg-gray-50 text-gray-900 border border-gray-100 rounded-2xl mb-4 text-lg outline-none focus:ring-2 focus:ring-purple-400 transition-all"
                    required
                  />
                  <textarea
                    ...
                    className="w-full px-6 py-4 bg-gray-50 text-gray-900 border border-gray-100 rounded-2xl mb-4 h-24 text-lg outline-none focus:ring-2 focus:ring-purple-400 transition-all"
                  />
                  <button
                    type="submit"
                    disabled={!selectedNoteFile}
                    className="w-full bg-purple-600 text-white py-5 rounded-2xl text-lg font-bold shadow-lg hover:bg-purple-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all uppercase tracking-widest"
                  >
                    Publish Resource
                  </button>
                </form>
                  ...
                  <div className="mt-8 pt-8 border-t border-purple-100">
                    ...
                    <div className="space-y-3 max-h-80 overflow-y-auto">
                      {notes.map((note) => (
                        <div
                          key={note._id}
                          className="flex justify-between items-center p-4 bg-gradient-to-r from-purple-50 to-indigo-50 rounded-2xl border border-purple-100"
                        >
                          <div>
                ...
                <div className="bg-white rounded-[40px] p-10 shadow-sm border-t-8 border-blue-500 border-x border-b border-gray-100">
                  <div className="flex justify-between items-center mb-6">
                    <h3 className="text-2xl font-bold flex items-center text-gray-800">
                      <HiOutlineClipboardList className="mr-3 text-blue-600 w-8 h-8" />{" "}
                      Assign New Task
                    </h3>
                    <Link
                  ...
                  <div
                    ...
                    className="border-2 border-dashed border-blue-100 rounded-[24px] p-10 flex flex-col items-center justify-center bg-blue-50/30 cursor-pointer mb-6 hover:bg-blue-50 transition-colors group"
                  >
                    <input
                      type="file"
                      ref={assignFileRef}
                      className="hidden"
                      onChange={(e) => setSelectedAssignFile(e.target.files[0])}
                      accept=".pdf,.doc,.docx,.txt"
                    />
                  ...
                  <form onSubmit={handleAddAssignment}>
                    <input
                      ...
                      className="w-full px-6 py-4 bg-gray-50 text-gray-900 border border-gray-100 rounded-2xl mb-4 text-lg outline-none focus:ring-2 focus:ring-blue-400 transition-all"
                      required
                    />
                    <textarea
                      ...
                      className="w-full px-6 py-4 bg-gray-50 text-gray-900 border border-gray-100 rounded-2xl mb-4 text-lg h-28 outline-none focus:ring-2 focus:ring-blue-400 transition-all"
                      required
                    />
                    <div className="flex flex-col mb-6">
                      <label className="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-2 ml-2">
                        Due Date
                      </label>
                      <input
                        type="date"
                        ...
                        className="w-full px-6 py-4 bg-gray-50 border border-gray-100 rounded-2xl text-lg text-gray-500 focus:ring-2 focus:ring-blue-400 transition-all"
                        required
                      />
                    </div>
                    <button
                  ...
                  <div className="mt-8 pt-8 border-t border-blue-100">
                    <h4 className="text-xl font-bold text-gray-800 mb-4">
                      Posted Assignments ({assignments.length})
                    </h4>
                    <div className="space-y-4 max-h-[400px] overflow-y-auto pr-2">
                      {assignments.length > 0 ? (
                        assignments.map((asm) => (
                          <div
                            key={asm._id}
                            className="p-6 bg-gray-50 rounded-3xl border border-transparent hover:border-gray-200 transition-all flex justify-between items-center group"
                          >
                            <div className="flex-1">
                              ...
                              <div className="flex items-center space-x-4 text-xs font-bold text-gray-400 uppercase tracking-widest">
                                <span className="flex items-center">
                                  <HiOutlineClock className="mr-1" /> Due:{" "}
                                  {new Date(
                                    asm.due_date || asm.submission_deadline,
                                  ).toLocaleDateString()}
                                </span>
                              </div>
                            </div>
      ...
      {isModalVisible && (
        <div className="fixed inset-0 flex justify-center items-center bg-black/40 backdrop-blur-sm z-50 p-4">
          <div className="bg-white p-12 rounded-[50px] shadow-2xl animate-in zoom-in duration-300 flex flex-col items-center border border-gray-100">
            <div className="w-20 h-20 bg-green-100 text-green-600 rounded-full flex items-center justify-center mb-6 text-4xl font-bold shadow-inner shadow-green-200/50">
              ✓
            </div>
            <h3 className="text-3xl font-bold text-gray-800 text-center">
              Attendance Logged
            </h3>
            <p className="text-[#64748b] text-center text-lg mt-2 font-medium">
              Class records have been updated.
            </p>
          </div>
        </div>
      )}
          ...
          <div className="bg-white w-full max-w-4xl max-h-[85vh] overflow-y-auto rounded-[50px] shadow-2xl animate-in zoom-in duration-300 border border-gray-100">
            ...
            <div className="p-10 space-y-8">
              <div className="bg-gray-50 p-6 rounded-[32px] border border-gray-100">
                <h4 className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-3 ml-2">
                ...
                {viewingAssignment.file_url && (
                  <a
                    href={viewingAssignment.file_url}
                    target="_blank"
                    rel="noreferrer"
                    className="mt-4 inline-flex items-center text-blue-600 font-bold text-sm bg-blue-50 px-4 py-2 rounded-xl hover:bg-blue-100 transition-all"
                  >
                ...
                viewingAssignment.questions.length > 0 && (
                  <div className="space-y-6">
                    ...
                    {viewingAssignment.questions.map((q, idx) => (
                      <div
                        key={idx}
                        className="p-8 bg-white border border-gray-100 rounded-[32px] hover:border-blue-100 transition-all"
                      >
                        ...
                        {q.options && q.options.length > 0 && (
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pl-12">
                            {q.options.map((opt, oIdx) => (
                              <div
                                key={oIdx}
                                className={`p-4 rounded-2xl border text-sm font-medium ${opt === q.correct_answer ? "bg-green-50 border-green-200 text-green-700" : "bg-gray-50 border-transparent text-gray-500"}`}
                              >
